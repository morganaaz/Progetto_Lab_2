                            ...
                            int charge, double width)
{
    if (Particle::FindParticle(newName) < 0){
        ...
        {
            width == 0?
            fParticleType[fNParticleType] = new ParticleType(newName, mass, charge)
...
double Particle::GetEnergy(double mass, double fpx, double fpy, double fpz) const{
...
int Particle::Decay2body(Particle &dau1,Particle &dau2) const {
  ...
  if(fIndexParticle > -1){ // add width effect
    ...
    massMot += fParticleType[fIndexParticle]->getWidth() * y1;
...
double c=4.;
double d=5.;
  double energy = GetEnergy(a,b,c,d);
